...
Sint ryu_d2exp_buffered_n   (F64 d, U32 precision, Char* result);
Sint ryu_d2fixed_buffered_n (F64 d, U32 precision, Char* result);
...
Sint ryu_d2fixed_buffered_n (F64 d, U32 precision, Char* result) {
    U64 bits = ryu_F64_to_U64(d);

    // Decode bits into sign, mantissa, and exponent.
    B32 ieeeSign = ((bits >> (RYU_DOUBLE_MANTISSA_BITS + RYU_DOUBLE_EXPONENT_BITS)) & 1) != 0;
    U64 ieeeMantissa = bits & ((1ull << RYU_DOUBLE_MANTISSA_BITS) - 1);
    U32 ieeeExponent = (U32) ((bits >> RYU_DOUBLE_MANTISSA_BITS) & ((1u << RYU_DOUBLE_EXPONENT_BITS) - 1));

    // Case distinction; exit early for the easy cases.
    if (ieeeExponent == ((1u << RYU_DOUBLE_EXPONENT_BITS) - 1u)) {
        return ryu_copy_special_str_printf(result, ieeeSign, ieeeMantissa);
    }
    ...
    if (ieeeExponent == 0) {
        ...
        e2 = (S32) ieeeExponent - RYU_DOUBLE_BIAS - RYU_DOUBLE_MANTISSA_BITS;
    ...
    if (e2 >= -52) {
        ...
        S32 len = (S32) ryu_lengthForIndex(idx);
        ...
        for (S32 i = len - 1; i >= 0; --i) {
            ...
            U32 digits = ryu_mulShift_mod1e9(m2 << 8, RYU_POW10_SPLIT[RYU_POW10_OFFSET[idx] + i], (S32) (j + 8));
            if (nonzero) {
                ryu_append_nine_digits(digits, result + index);
                index += 9;
            } else if (digits != 0) {
                U32 olength = ryu_decimalLength9(digits);
                ryu_append_n_digits(olength, digits, result + index);
                index += olength;
                nonzero = true;
            }
        }
    ...
    if (e2 < 0) {
        ...
        for (; i < blocks; ++i) {
            ...
            if (i < blocks - 1) {
                ...
                if (lastDigit != 5) {
                    ...
                    S32 requiredTwos = -e2 - (S32) precision - 1;
                    B32 trailingZeros = requiredTwos <= 0
                        || (requiredTwos < 60 && ryu_multipleOfPowerOf2(m2, (U32) requiredTwos));
                    roundUp = trailingZeros ? 2 : 1;
                }
                if (maximum > 0) {
                    ryu_append_c_digits(maximum, digits, result + index);
                    index += maximum;
                }
                break;
            }
        ...
        if (roundUp != 0) {
            Sint roundIndex = index;
            Sint dotIndex = 0; // '.' can't be located at index 0
            while (true) {
                --roundIndex;
                Char c;
...
Sint ryu_d2exp_buffered_n (F64 d, U32 precision, Char* result) {
    ...
    // NOTE(naman): Start copying here
    B32 printDecimalPoint = precision > 0;
    ++precision;
    Sint index = 0;
    if (ieeeSign) {
        result[index++] = '-';
    ...
    if (e2 >= -52) {
        ...
        for (S32 i = len - 1; i >= 0; --i) {
            ...
            // Temporary: j is usually around 128, and by shifting a bit, we push it to 128 or above, which is
            // a slightly faster code path in ryu_mulShift_mod1e9. Instead, we can just increase the multipliers.
            digits = ryu_mulShift_mod1e9(m2 << 8, RYU_POW10_SPLIT[RYU_POW10_OFFSET[idx] + i], (S32) (j + 8));
            if (printedDigits != 0) {
                ...
                availableDigits = ryu_decimalLength9(digits);
                exp = i * 9 + (S32) availableDigits - 1;
                if (availableDigits > precision) {
                    break;
                }
                if (printDecimalPoint) {
                    ryu_append_d_digits(availableDigits, digits, result + index);
    ...
    if (e2 < 0 && availableDigits == 0) {
        ...
        for (S32 i = RYU_MIN_BLOCK_2[idx]; i < 200; ++i) {
            ...
            if (printedDigits != 0) {
                ...
                availableDigits = ryu_decimalLength9(digits);
                exp = -(i + 1) * 9 + (S32) availableDigits - 1;
                if (availableDigits > precision) {
                    break;
                }
                if (printDecimalPoint) {
    ...
    U32 maximum = precision - printedDigits;
    ...
    if (availableDigits > maximum) {
        for (U32 k = 0; k < availableDigits - maximum; ++k) {
            lastDigit = digits % 10;
            digits /= 10;
        }
    }
    ...
    if (lastDigit != 5) {
        ...
        if (rexp < 0) {
            ...
            trailingZeros = trailingZeros && ryu_multipleOfPowerOf5(m2, (U32) requiredFives);
        }
        roundUp = trailingZeros ? 2 : 1;
    }
    ...
    if (printedDigits != 0) {
        ...
        if (printDecimalPoint) {
            ryu_append_d_digits(maximum, digits, result + index);
...
# endif
    ...
    if (exp >= 100) {
        S32 c = exp % 10;
        memcpy(result + index, RYU_DIGIT_TABLE + 2 * (exp / 10), 2);
        result[index + 2] = (Char) ('0' + c);
        index += 3;
    } else {
        memcpy(result + index, RYU_DIGIT_TABLE + 2 * exp, 2);
        index += 2;
    }
